...
The `thinking` tool lets the agent share thoughts or reasoning before it continues. At session start, it is active by default only when the active model uses the `openai-codex` provider. While active, it encourages the agent to expose internal reasoning when that would help the user inspect or correct assumptions, decisions, or changes in direction. The extension may queue a gentle hidden nudge after both five completed thinking blocks and 60 seconds have elapsed since the previous nudge or `thinking` tool call. Either event resets both guards. The agent should copy those internal thoughts verbatim when available and permitted. It should paraphrase or summarize only when instructions or system limitations prevent verbatim disclosure, while preserving key considerations and decisions. Thoughts should use multiple short lines, stay within the TUI width, and wrap at roughly 160 characters when that width is unknown.
