...
At session start, the tool is active by default only when the active model uses the `openai-codex` provider. While active, it encourages the agent to expose internal reasoning when that would help the user inspect or correct assumptions, decisions, or changes in direction, copying those thoughts verbatim when available and permitted. Paraphrasing or summarization is only a fallback when instructions or system limitations prevent verbatim disclosure. The extension may queue a gentle hidden nudge after both five completed thinking blocks and 60 seconds have elapsed since the previous nudge or `thinking` tool call. Either event resets both guards. Thoughts use multiple short lines and wrap to the TUI width, or roughly 160 characters when unknown. Use `/thinking` to flip its state, or `/thinking on` and `/thinking off` to set it explicitly, without affecting other tools.
